      ...
      <nav className="flex-1 min-h-0 overflow-y-auto" aria-label="Plans">
        <ul>
          {plans && plans.map((plan) => (
            ...
            >
              <div
                className={`relative hover:cursor-pointer px-6 py-5 m-2 rounded-lg flex items-center space-x-3 bg-slate-50 hover:bg-gray-100 shadow-lg outline outline-1 outline-gray-100 ${plan.id === currentlySelected ? "border-l-8 border-blue-400 ml-3 mr-1" : ""}`}
